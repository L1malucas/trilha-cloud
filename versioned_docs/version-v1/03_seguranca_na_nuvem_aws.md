...
> `[PRINT]` Passo a passo para capturar: no Console, buscar "IAM" e abrir o serviço. Capturar a tela inicial ("Dashboard"), mostrando o resumo de recursos (Users, User groups, Roles, Policies) e, se visível, o painel de "Security recommendations" ou "IAM Access Analyzer".
...
> `[PRINT]` Passo a passo para capturar: no Console, buscar "Security Hub" e abrir o serviço (pode ser necessário clicar em "Go to Security Hub" ou habilitar o serviço na primeira vez, sem precisar concluir nenhuma configuração paga). Capturar a tela do resumo/dashboard, mostrando os cartões de contagem de achados por severidade, mesmo que estejam todos zerados por não haver histórico ainda.
